var whiteDiceNrs = [ "white-one", "white-two", "white-three", "white-four",
		"white-five", "white-six" ];
var brownDiceNrs = [ "brown-one", "brown-two", "brown-three", "brown-four",
		"brown-five", "brown-six" ];
...
function rollDice(string) {
	for ( var i = 0; i < 5; i++) {
		...
		console.log('newWhiteNr: ' + newWhiteNr + " "
				+ whiteDiceNrs[newWhiteNr]);
		console.log('newBrownNr: ' + newBrownNr + " "
				+ brownDiceNrs[newBrownNr]);
		if (currentWhiteNr != whiteDiceNrs[newWhiteNr]) {
			$('#whiteDiceNr1').switchClass(currentWhiteNr,
					whiteDiceNrs[newWhiteNr], 125);
		}
		if (currentBrownNr != brownDiceNrs[newBrownNr]) {
			$('#brownDiceNr1').switchClass(currentBrownNr,
					brownDiceNrs[newBrownNr], 125);
		}
...
function rollWhiteDice(string, value) {
	for ( var i = 0; i < 5; i++) {
		var newWhiteNr = Math.floor((Math.random() * 6));
		console
				.log(string + ': ' + newWhiteNr + " "
						+ whiteDiceNrs[newWhiteNr]);
		if (currentWhiteNr != whiteDiceNrs[newWhiteNr]) {
			$(string)
					.switchClass(currentWhiteNr, whiteDiceNrs[newWhiteNr], 125);
		}
		currentWhiteNr = whiteDiceNrs[newWhiteNr];
	}
...
function rollBrownDice(string, value) {
	for ( var i = 0; i < 5; i++) {
		var newBrownNr = Math.floor((Math.random() * 6));
		console
				.log(string + ': ' + newBrownNr + " "
						+ brownDiceNrs[newBrownNr]);
		if (currentBrownNr != brownDiceNrs[newBrownNr]) {
			$(string)
					.switchClass(currentBrownNr, brownDiceNrs[newBrownNr], 125);
		}
		currentBrownNr = brownDiceNrs[newBrownNr];
	}
...
function startGameRolling(whiteValue, brownValue) {
	$("#game-board")
			.append(
					'<div id="whiteDiceNr1Container" class="dice dice-white"><div id="whiteDiceNr1" class="number white-one"></div></div>');
	$("#game-board")
			.append(
					'<div id="brownDiceNr1Container" class="dice dice-brown"><div id="brownDiceNr1" class="number brown-three"></div></div>');
...
function whiteRolling(first, second) {
	...
	$("#game-board")
			.append(
					'<div id="whiteDiceNr2Container" class="dice dice-white"><div id="whiteDiceNr2" class="number white-one"></div></div>');
...
function brownRolling(first, second) {
	...
	$("#game-board")
			.append(
					'<div id="brownDiceNr2Container" class="dice dice-brown"><div id="brownDiceNr2" class="number brown-three"></div></div>');
